...
// ── FETCH PRODUCTS GROUPED BY BRANCH ──────────────────────────────────────────
$products_by_branch = [];
$prod_query = $db->query("
    SELECT p.id, p.product_id, p.name, p.selling_price, s.branch_id 
    FROM products p
    JOIN stocks s ON p.id = s.product_id
    ORDER BY p.name
");
while ($r = $prod_query->fetch_assoc()) {
    $products_by_branch[$r['branch_id']][] = [
        'id'            => $r['id'],
...
<script>
// Load the branch-product mapping generated by PHP
const productsByBranch = <?= json_encode($products_by_branch) ?>;
const branchSelect     = document.getElementById('branchSelect');
const itemsContainer   = document.getElementById('items-container');
let rowCount = 0;

// When the user changes the branch, wipe the rows so they don't accidentally check out items from the wrong canteen!
branchSelect.addEventListener('change', function() {
    itemsContainer.innerHTML = '';
    rowCount = 0;
    updateTotal();
    
    // Automatically add a fresh blank row if a valid branch was selected
    if (this.value) {
        addRow();
    }
});

// Helper function to insert the correct options into a specific row's dropdown
function populateProductDropdown(selectElement) {
    selectElement.innerHTML = '<option value="">Select product…</option>';
    const branchId = branchSelect.value;
    
    // If no branch is selected or no products exist for this branch, exit
    if (!branchId || !productsByBranch[branchId]) return;

    // Loop through the items for this specific branch and add them as <options>
    productsByBranch[branchId].forEach(p => {
        const opt = document.createElement('option');
        opt.value = p.id;
        opt.dataset.price = p.selling_price;
        opt.textContent = `${p.product_id} – ${p.name} (₱${parseFloat(p.selling_price).toFixed(2)})`;
...
function addRow() {
    if (!branchSelect.value) {
        alert("Please select a branch before adding products.");
        return;
    }
    ...
    // Load the correct items into this new row
    populateProductDropdown(sel);
...
// Auto-run on page load ONLY if a branch is already pre-selected (like for Cashiers)
if (branchSelect.value) {
    addRow();
}
</script>
